                    ...
                    if j.lower() in text.lower():
                        ...
                        for k in AT_json['features']:
                            if (k['properties']['Name'].strip() == i[0].strip()):
                                ...
                                return new_x, new_y, dictionary
            elif i.lower() in text.lower():
                dictionary[row[0]] += 1
                for j in NP_json['features']:
                    if (j['properties']['UNIT_NAME'].strip() == i.strip()):
...
def parseTweet(tweet, NPAT_dict, ID_COUNT_dict):
    ...
    NPAT = [['Acadia National Park', 'AcadiaNPS', 'Acadia100', 'AcadiaNPS', 'acadianationalPark', ['Cadillac Mountain', 'cadillacmountain'], ['Thunder Hole', 'thunderhole'], ['Schoodic Peninsula', 'schoodicpoint', 'schoodic'], ['Jordan Pond', 'Jordonpond']],
            ...
            ['Olympic National Park', 'OlympicNP', 'OlympicNP', 'OlympicNationalPark', 'Olympic', ['Hoh Rain Forest', 'HohRainForest'], ['Hurricane Ridge', 'HurricaneRidge'], ['Lake Crescent', 'LakeCrescent'], ['Olympic National Forest', 'OlympicNationalForest'], ['Rialto Beach', 'RialtoBeach']],
    ...
    tw_text = tweet['text']
    cor_x, cor_y, NPAT_dict = countNPAT(NPAT, tw_text, NPAT_dict, attraction_json, nationalpark_json)
    ...
    return cor_x, cor_y, senti_value, NPAT_dict, ID_COUNT_dict
...
consumer_key_1 = "ygSJ8rlgsGoXXod5maFtyacWg"
consumer_secret_1 = "TVLnApqyYLT3XfZAwunA2Xc3YAZL0isBZ71uZXDaLUGNZ44PWo"

access_token_2 = "2717764046-85r7oU7kwd8QpHFMxggaFxvtsBgF2NoHtB41KKa"
access_token_secret_2 = "V6XuyCLvE2mS6hVVCK4Ha4rkl3ii505ESuKC5hOS95y1c"
consumer_key_2 = "YnHhRCVhhf3VYe0L1eEEslNdM"
consumer_secret_2 = "I5HB9RiUxaf5pFLlXT4T090gfdJCopkDpDHHv7ryl47auPSU8J"
...
class StdOutListener1(StreamListener):
    def on_data(self, data):
        ...
        text = data['text']
        tw_cor_x, tw_cor_y, tw_senti_value, tw_NPAT_dict, tw_ID_COUNT_dict = parseTweet(data, tw_NPAT_dict, tw_ID_COUNT_dict)
        if ((tw_cor_x != 0) and (tw_cor_y != 0)):
            socketio.emit('streamTweets1',
                          {'user_name': name, 'stream_text': text, 'X_cor': tw_cor_x, 'Y_cor': tw_cor_y, 'count_dict': tw_ID_COUNT_dict, 'sentimen': tw_senti_value},
                          namespace='/npTweet')
...
if __name__ == "__main__":
    socketio.run(app, debug=True, host="0.0.0.0", port=9999)  # make it externally visible
